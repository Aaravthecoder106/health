...
with st.sidebar:
    ...
    st.markdown("<p style='color: #64748b; font-size: 0.8rem; letter-spacing: 1px;'>CHOOSE AI ENGINE VERSION</p>", unsafe_allow_html=True)
    model_version = st.selectbox(
        "",
        ["Gemini 2.5 Flash (Engine 1)", "Gemini 2.5 Pro (Engine 2)"],
        index=0
    )
...
with col2:
    ...
    if st.button("DECODE JARGON"):
        if not user_query.strip() and not file_path:
            ...
            st.markdown(f"""
            <div style="background: rgba(6, 22, 45, 0.65); padding: 25px; border-radius: 14px; border-left: 4px solid #00f2fe; box-shadow: 0 15px 40px rgba(0,0,0,0.6); backdrop-filter: blur(14px); color: #e2e8f0; border-top: 1px solid rgba(0, 210, 255, 0.1);">
                {response}
            </div>
            """, unsafe_allow_html=True)
